...
export function ForList (props) {
    ...
    useEffect(
        () => {
            ...
            if (props.cardsList) {
                const forCards = props.cardsList.filter((card) => card.position === 'FOR');
                const forScarcitySelected = forCards.filter(
                    (card) => {
                ...
                const sortByScore = forScarcitySelected.sort((cardA, cardB) => cardB.score - cardA.score)
