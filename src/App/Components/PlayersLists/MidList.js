...
export function MidList (props) {
    ...
    useEffect(
        () => {
            ...
            if (props.cardsList) {
                const midCards = props.cardsList.filter((card) => card.position === 'MID');
                const midsScarcitySelected = midCards.filter(
                    (card) => {
                ...
                const sortByScore = midsScarcitySelected.sort((cardA, cardB) => cardB.score - cardA.score)
